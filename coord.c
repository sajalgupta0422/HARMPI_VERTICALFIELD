...
void set_points()
{
  ...
  dx[3] = lenx[3]/mpi_ntot[3] ; //phi
    ...
    for(dim = 1; dim < NDIM; dim++) {
        mpi_startx[dim] = startx[dim] + mpi_coords[dim]*mpi_ntile[dim]*dx[dim] ;
        mpi_stopx[dim]  = startx[dim] + (mpi_coords[dim]+1)*mpi_ntile[dim]*dx[dim] ;
    }
#else
...
Maintains proper boundary conditions for magnetic fields near the poles
Controls mass flux at boundaries to prevent unwanted inflow/outflow
Handles these conditions properly in MPI parallel environments (note the  mpi_coords checks)
F1, F2, and F3 are fluxes in different directions. From  defs.h:
F1: Radial flux (x1 direction); F2: Polar flux (x2 direction); F3: Azimuthal flux (x3 direction)
double  a_F1[N1M][N2M][N3M][NPR] ;	
double  a_F2[N1M][N2M][N3M][NPR] ;	
double  a_F3[N1M][N2M][N3M][NPR] ;	
Each flux array contains components for different conserved quantities (density, energy, momentum, magnetic fields, etc.) as indicated by the last dimension [NPR]
*/
...
//#pragma omp parallel for schedule(static,MY_MAX((N1+1)*(N3+1)/nthreads,1)) collapse(2) default(none) shared(F1,F2,nthreads) private(i,k,m)
        /* Below for loop is doing:
        At θ boundaries (j=-1 and j=N2):
        Reflects the θ-component of magnetic field (B2) with opposite sign
        For F1 (radial) and F3 (φ) fluxes
        */
        for(i=-D1;i<N1+D1;i++) {
          for(k=-D3;k<N3+D3;k++) {

            if(mpi_coords[2] == 0) { //handles the lower θ boundary (near θ = 0):
              F1[i][-1][k][B2] = -F1[i][0][k][B2] ;
              F3[i][-1][k][B2] = -F3[i][0][k][B2] ;
            }
            if(mpi_coords[2] == mpi_dims[2]-1) { //handles the upper θ boundary (near θ = π):
            ...
            if(INFLOW ==  0){
              /*If INFLOW=0:
              Sets all components of F2 (θ-direction flux) to zero at θ boundaries*/
...
//#pragma omp parallel for schedule(static,MY_MAX((N2+1)*(N3+1)/nthreads,1)) collapse(2) default(none) shared(F1,nthreads) private(j,k)
        /* Below for loop is doing:
        Sets all components of F1 (radial flux) to zero at radial boundaries by
        Prevents inflow at inner boundary by setting positive radial density flux to zero
        Prevents outflow at outer boundary by setting negative radial density flux to zero
        */
        for(j=-D2;j<N2+D2;j++) {
          for(k=-D3;k<N3+D3;k++) {
...
{
  /*
Rescales primitive variables based on coordinate direction and location
 * 
 * Parameters:
 *   pr    - Array of primitive variables to rescale
 *   which - Direction of rescaling (FORWARD or REVERSE)
 *   dir   - Coordinate direction (1=radial, 2=theta, 3=phi)
 *   ii    - i-index of grid point
 *   jj    - j-index of grid point  
 *   kk    - k-index of grid point
 *   face  - Face of cell (e.g., CENT for cell center)
 *   geom  - Geometric information at the point
  */
  ...
  if (which == FORWARD) {	// rescale before interpolation; Multiplies variables by scale factors before interpolation
    PLOOP pr[m] *= scale[m];
  } else if (which == REVERSE) {	// unrescale after interpolation; Divides variables by scale factors after interpolation
    PLOOP pr[m] /= scale[m];
  } else {
...
void vofx_sjetcoords( double *X, double *V )
{
  ...
  ror0nudisk = pow( (fakerdisk - global_rsjet*Rin)/global_r0grid, global_jetnu2/2 );
  ror0nujet = pow( (fakerjet - global_rsjet*Rin)/global_r0grid, global_jetnu2/2 );
  thetadisk = thetaofx2( X[2], ror0nudisk );
  thetajet = thetaofx2( X[2], ror0nujet );
  V[2] = fac*thetajet + (1 - fac)*thetadisk;
#elif(BL==3) //SASHA's simple
    double r1disk, r2disk, r1jet, r2jet, r1, dr;
    fac = Ftrgen( fabs(X[2]), global_fracdisk, 1-global_fracjet, 0, 1 );
    
    r1disk = mins( V[1]/global_r0disk, 1. , 0.5 ) * (global_r0disk/global_r0grid);
    r2disk = V[1]/(r1disk*global_r0grid);
    dr = global_rdiskend/global_r0disk;
    r2disk = mins( r2disk, dr, 0.5*dr );
    ...
    ror0nudisk = pow( r1disk, 0.5*global_disknu1) * pow(r2disk, 0.5*global_disknu2);;
...
r = exp(x1) + R0   (for x1 ≤ x1br)
r = exp(x1 + cpow2 * pow(x1-x1br, npow2)) + R0   (for x1 > x1br)
Where:
x1 is the internal radial coordinate
R0 is the inner radius offset
x1br is the break radius where the grid spacing changes
cpow2 and npow2 control the hyperexponential grid spacing beyond x1br

θ = (π/2)(1 + x2) + ((1 - hslope)/2) * sin(π(1 + x2))
Where:
x2 is the internal polar coordinate (-1 ≤ x2 ≤ 1)
hslope controls grid concentration toward the equatorial plane
This maps x2 range [-1,1] onto θ range [0,π]

Then modified by cylindrification:
if r < global_x10:
    θ = π/2
else if |cos(θ)| > global_x20:
    f = (r-global_x10)/(10-global_x10)
    f = min(f, 1)
    θ = π/2 + f*(θ - π/2)
else:
    θ = θ (unchanged)
...
Together, these parameters control the cylindrification process:
 global_x10 determines "how far out" the cylindrification extends radially
 global_x20 determines "how close to the pole" the cylindrification affects in θ

The cylindrification process makes the grid more cylindrical near the axis by:
Making cells fully cylindrical (θ = π/2) for r < global_x10
Creating a smooth transition region where global_x20 < |cos(θ)| and r > global_x10
Leaving the grid spherical elsewhere

φ = x3


Great! Hmm, could you tell me whether θ completely ranges from 0 to π? Or does it have a little offset, like 0 + δθ to π - δθ, where this δθ may depend on radius?
Looking at the code and the coordinate transformations, θ does not completely range from 0 to π. There is indeed an offset δθ that depends on radius.
1. First, in  decs.h, we see  COORDSINGFIX is defined:
#define COORDSINGFIX 1
// whether to move polar axis to a bit larger theta
// theta value where singularity is displaced to
#define SINGSMALL (1.E-20)

2. The cylindrification process in vofx_cylindrified further modifies this by:
Making θ = π/2 for r < global_x10
Creating a transition region where |cos(θ)| > global_x20
The parameter global_x20 is set in  init.c:
...
//The innermost grid cell is evolved inaccurately whether you resolve it or not, and it will be fixed by POLEFIX (see bounds.c).

So yes, θ effectively ranges from (0 + δθ) to (π - δθ), where δθ varies with radius:
At small r < global_x10: δθ is large enough to force θ = π/2
In the transition region: δθ smoothly varies
At large r: δθ is small but non-zero (SINGSMALL)
This is done intentionally to avoid the coordinate singularity at the poles (θ = 0 and θ = π) while maintaining accuracy elsewhere. The comment in  init.c confirms this is handled by the  POLEFIX routines in  bounds.c for numerical stability
*/