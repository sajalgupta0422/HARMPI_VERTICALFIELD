   ...
   primtoU_g(): 
       ...
       -- assumes that the conserved and primitive variables are defined ala HARM:
         ...
         U =  |  T^t_\mu + rho u^t |  sqrt(-det(g_{\mu\nu}))
             ...
             /    rho        \
	 P = |    uu (internal enegry)        |
             | \tilde{u}^i   = Spatial drift velocity relative to ZAMO|
             \   B^i         /

SG : \tilde{u}^i = u^i - γ η^i,  η^μ = (1/α,-β^i/α) is 4-contra velocity of ZAMO observer
...
void primtoU_g(
  ...
  alpha = 1.0/sqrt(-gcon[0][0]); //time lapse factor

  ucon_calc_g(prim,gcov,gcon,ucon) ; //geting contravarinat 4 velocity
  lower_g(ucon,gcov,ucov) ; //geting covariant 4 velocity
  ncov_calc(gcon,ncov) ; //getting ZAMO observer 4- covariant velocity; η_μ = (-α,0,0,0)

  gamma = -ncov[0]*ucon[0] ; //u^t = γ/α; η_0 = -α

  bcon_calc_g(prim,ucon,ucov,ncov,bcon) ; //getting contravariant magnetic field
  lower_g(bcon,gcov,bcov) ; //getting covariant magnetic field
  ...
  rho0 = prim[RHO] ;
  u = prim[UU] ; //internal energy
  p = pressure_rho0_u(rho0,u) ; //return (γ-1)*u, where u is internal energy
  w = rho0 + u + p ; //enthalpy

  // Now set the conserved variables themselves, using HARM's definition:
  U[RHO] = ucon[0]*rho0 ; //ρu^t is conserved, and it is U[RHO]
  ...
  for(i = 0; i < NPR; i++ ) {
    U[i] *= gdet; //multiplying by sqrt(-det(g_{\mu \nu}))
  }
...
SG : I think from Mckinney(2206) \tilde{u}^i = u^i - γ η^i,  η^μ = (1/α,-β^i/α) is 4-contra velocity of ZAMO observer 
       -- also requires the metric and inverse metric;

       -- assumes:

             /    rho        \
	 P = |    uu (internal energy)        |
             | \tilde{u}^i   |
             \   B^i         /
		 ...
		 FTYPE ucon[NDIM])
{
  ...
  gamma = sqrt(1. + u_tilde_sq) ;//γ = \sqrt{1 + q^2}; q^2 = g_{ij} \tild{u}^i \tild{u}^j

  lapse = sqrt(-1./gcon[0][0]) ; //or alpha

  for(i=0;i<NDIM;i++) ucon[i] = u_tilde_con[i] - lapse*gamma*gcon[0][i] ; ////getting u^i = \tild{u}^i + γη^i, where η^μ = (1/α,-β^i/α) is 4-contra velocity of ZAMO observer
...
int gamma_calc_g(FTYPE *pr, FTYPE gcov[NDIM][NDIM], FTYPE *gamma)
{
  ...
  *gamma = sqrt(1. + utsq) ; //γ = \sqrt{1 + q^2}; q^2 = g_{ij} \tild{u}^i \tild{u}^j
