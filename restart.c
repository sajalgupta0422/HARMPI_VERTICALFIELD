...
void restart_write(int dumpno)
{
  ...
  if (!DO_PARALLEL_WRITE || i_am_the_master) {
      ...
      fprintf(fp, FMT_INT_OUT, 0);
#else
      //note that always write out with ghost cells for restart dumps
      //since dumping the entire array at once
...
#   if(EVOLVEVPOT)
      //this does not write the outermost (i=N1+D1,etc) cells but they probably are zero, so may be OK. CHECK!
      n+= write_to_rdump(0, NULL, rdump_buffer+n, vpot[1][i][j][k]);
      n+= write_to_rdump(0, NULL, rdump_buffer+n, vpot[2][i][j][k]);
      n+= write_to_rdump(0, NULL, rdump_buffer+n, vpot[3][i][j][k]);
#   endif
  }
...
# if(EVOLVEVPOT)
  for (m=1; m<=3; m++) {
    ntowrite = 1L*(N1+D1)*(N2+D2)*(N3+D3)*1;
    nhavewritten = fwrite( vpot[m], sizeof(double), ntowrite, fp );
    if( nhavewritten != ntowrite ) {
      fprintf(stderr, "MPI rank %d: Could not write to restart file: nhavewritten = %ld, ntowrite = %ld, wrong format or corrupted file?\n", mpi_rank, (long)nhavewritten, (long)ntowrite);
      fflush(stderr);
#if MPI
...
int restart_init()
{
  ...
  ************************************************************************/
  ...
  //DTr01 = 1760. ; /* restart file frequ., in timesteps */

  //reset KEL4B to KEL4E
  /* fprintf(stderr,"Resetting KELDIS, KEL4B, KEL4C to KEL4E, KEL4A to KEL4D...\n") ; */
  /* ZSLOOP(-N1G,N1+N1G-1,-N2G,N2+N2G-1,-N3G,N3+N3G-1) { */
  /*   PLOOP p[i][j][k][KEL4] = p[i][j][k][KEL4E]; */
  /*   PLOOP p[i][j][k][KELDIS] = p[i][j][k][KEL4E]; */
  /*   PLOOP p[i][j][k][KEL4A] = p[i][j][k][KEL4D]; */
  /*   PLOOP p[i][j][k][KEL4B] = p[i][j][k][KEL4E]; */
  /*   PLOOP p[i][j][k][KEL4C] = p[i][j][k][KEL4E]; */
  /* } */
...
int restart_read(int dumpno)
{
  ...
  *************************************************************/
  ...
  fscanf_and_bcast(fp, "%lf", &global_disknu1     );
  fscanf_and_bcast(fp, "%lf", &global_disknu2     );
...
#if MPI && DO_PARALLEL_WRITE 
  {
    ...
    ZLOOP {
      PLOOP {
        ...
        // /* Set electron internal energy equal to total internal energy */
        // double ke4 = (4./3.-1.)*p[i][j][k][UU]/pow(p[i][j][k][RHO],4./3.);
        // double ke5 = (5./3.-1.)*p[i][j][k][UU]/pow(p[i][j][k][RHO],5./3.);
        ...
        // p[i][j][k][KEL4E] = ke4;
        // p[i][j][k][KELDIS]  = ke4;
        // p[i][j][k][KEL5]  =ke5; // p[i][j][k][KTOT];

#     if(EVOLVEVPOT)
        //zero out the zeroth component, which is not used
        vpot[0][i][j][k] = 0.;
        for(m=1; m<=3; m++) {
          vpot[m][i][j][k] = rdump_buffer[n];
          n++;
        }
#     endif
    }
...
# if(EVOLVEVPOT)
    for (m=1; m<=3; m++) {
      ntoread = 1L*(N1+D1)*(N2+D2)*(N3+D3)*1;
      nhaveread = fread( vpot[m], sizeof(double), ntoread, fp );
      if( nhaveread != ntoread || feof(fp) ) {
        fprintf(stderr, "MPI rank %d: Restart file ended prematurely: nhaveread = %ld, ntoread = %ld, wrong format or corrupted file?\n", mpi_rank, (long)nhaveread, (long)ntoread);
        fflush(stderr);
#if MPI
