...
double mpi_buf_send[NDIM][2][(NMAX+2*NG)*(NMAX+2*NG)*NG*NPR+NPTOT];
double mpi_buf_recv[NDIM][2][(NMAX+2*NG)*(NMAX+2*NG)*NG*NPR+NPTOT];
//pflag
...
MPI_Datatype sdump_file_type, sdump_cell_type;
...
int i_am_the_master;
double mpi_startx[NDIM], mpi_stopx[NDIM];
...
double  a_uelvar[N1M][N2M][N3M]; /* electron internal energy */
double  a_thetaold[N1M][N2M][N3M]; /* temperature at previous time step for guess */
double  a_qdot[N1M][N2M][N3M]; /* source term diagnostics */
#if(DOQDOTAVG)
double  a_qdotavg[N1M][N2M][N3M]; /* source term diagnostics */
double  a_feqdotavg[N1M][N2M][N3M]; /* source term diagnostics */
#endif
double  a_qisosq[N1M][N2M][N3M]; /* source term diagnostics */
double  a_qisodotb[N1M][N2M][N3M]; /* source term diagnostics */

double a_uelarray[N1M][N2M][N3M][KELDIS-KEL4 +1]; /* space for electron internal energy (used for failure points) */
double a_du_flr[N1M][N2M][N3M]; /* energy added by floor */



double complex a_delvx[N1M][N2M];
double complex a_delvy[N1M][N2M];
...
#if(EVOLVEVPOT)
double vpot[NDIM][N1+D1][N2+D2][N3+D3] ; //OPTMARK: could reduce NDIM to 1 in 2D and eliminate completely in 1D
#endif
...
double (*qisosq)[N2M][N3M];
double (*qisodotb)[N2M][N3M];
...
double (* uelvar)[N2M][N3M];
double (* thetaold)[N2M][N3M];
double (* qdot)[N2M][N3M] ;
#if(DOQDOTAVG)
double (* qdotavg)[N2M][N3M] ;
double (* feqdotavg)[N2M][N3M] ;
#endif
double complex (*delvx)[N2M];
double complex (*delvy)[N2M];

double (*   uelarray)[N2M][N3M][KELDIS-KEL4 +1] ;
double (*   du_flr)[N2M][N3M] ;
...
double cour ;
double dV,dx[NDIM],startx[NDIM],lenx[NDIM] ;
double dt ;
double t,tf ;
double rcurr,hcurr ;
...
double tdump,trdump,timage,tlog,tavgstart ;
...
double global_disknu1;
double global_disknu2;
